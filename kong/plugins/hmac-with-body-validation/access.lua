...
local function validate_request_body(headers)
  req_read_body()
  ...
  return str.to_hex(sha256:final()) == headers[DIGEST_HEADER]
end
...
local function do_authentication(conf)
  ...
  -- If either authorization header or digest header is missing then, return 401
  if not (headers[AUTHORIZATION] or headers[PROXY_AUTHORIZATION]) or not headers[DIGEST_HEADER] then
  ...
  -- Validate integrity of request body.
  if not validate_request_body(headers) then
    return false, {status = 403, message = "HMAC signature does not match" }
  end
